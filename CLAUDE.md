...
- **Typst books work on Quarto ≥1.9** (image is 1.9.38). Render: `quarto render --profile typst --to typst --output-dir ./typst-out` (profile `_quarto-typst.yml` adds `format: typst`). Output is `typst-out/` (gitignored). Caveats: typst's native `.bib` parser is stricter than biblatex — **no duplicate keys**, and `url` must be a real URL (not a bare DOI like `10.xxx`); the LaTeX-only `tex-hacks/` + `callout_custom_pdf.lua` don't apply, so `bloc_*` callouts render as plain blocks. **LaTeX PDF (`--profile production`) stays canonical** — typst is experimental/optional.
