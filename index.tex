...
\floatname{codelisting}{Bloc de code}
...
\item
  comprendre les structures de base du langage Python (listes, tuples,
...
\item
  manipuler un tableau \texttt{NumPy}.
\end{itemize}

\end{tcolorbox}

Ce chapitre est aussi disponible sous la forme d'un notebook Python sur
Google Colab :

\href{https://colab.research.google.com/github/sfoucher/TraitementImagesPythonVol1/blob/main/notebooks/00-PriseEnMainPython.ipynb}{\pandocbounded{\includegraphics[keepaspectratio]{images/colab.png}}}
...
\bottomrule\noalign{}
\endlastfoot
\href{https://www.python.org/downloads/}{CPython} & L'implémentation
officielle « vanille » & La compatibilité et la conformité aux
standards \\
\href{https://www.anaconda.com/download}{Anaconda} & Livrée avec de
nombreuses bibliothèques scientifiques & L'analyse de données et
l'apprentissage automatique (\emph{machine learning}) \\
\href{https://docs.anaconda.com/miniconda/miniconda-install/}{Miniconda}
& Version légère ; on ajoute les bibliothèques au besoin & Un
environnement minimal et contrôlé \\
\href{https://pypy.org/}{PyPy} & Implémentation optimisée pour la
vitesse d'exécution & Les programmes gourmands en calcul \\
\end{longtable}
}
...
comme choisir entre une glace simple ou une glace royale (banana split)
élaborée.
...
Un IDE (\emph{Integrated Development Environment}) est comme une boîte à
outils complète pour les programmeurs, vous trouverez :
...
\bottomrule\noalign{}
\endlastfoot
\href{https://www.jetbrains.com/pycharm/}{PyCharm} & IDE complet &
Autocomplétion, débogage intégré ; idéal pour les grands projets
(gourmand en ressources) \\
\href{https://code.visualstudio.com/}{Visual Studio Code} & Éditeur
extensible & Gratuit, léger, personnalisable par extensions \\
\href{https://www.spyder-ide.org/}{Spyder} & IDE scientifique & Libre et
gratuit, orienté calcul scientifique \\
\href{https://jupyter.org/}{Jupyter} & Notebook & Mélange code, texte et
visualisations ; gratuit sur Colab/Kaggle (reproductibilité limitée) \\
\href{https://marimo.io/}{Marimo} & Notebook réactif & Réexécute
automatiquement les cellules dépendantes ; évite l'état caché \\
\end{longtable}
}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{N'installez pas la toute dernière version de Python} : Il est
...
\item
  ...
  compartiments séparés pour chaque projet. Cela évite les conflits
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{conda}\NormalTok{ create }\AttributeTok{{-}n}\NormalTok{ test python=3.10}
...
\item
  ...
  \href{https://www.python.org/downloads/}{CPython}, sans
  \texttt{conda}. Le module \texttt{venv} de la bibliothèque standard
  crée l'environnement et \texttt{pip} installe ensuite les
  bibliothèques :
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ venv test}
\BuiltInTok{source}\NormalTok{ test/bin/activate       }\CommentTok{\# Windows : test\textbackslash{}Scripts\textbackslash{}activate}
...
Python manipule quatre structures de données fondamentales : les listes,
les tuples, les ensembles et les dictionnaires.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Une liste des bandes spectrales d\textquotesingle{}une image (analogie télédétection)}
\NormalTok{bandes }\OperatorTok{=}\NormalTok{ [}\StringTok{"bleu"}\NormalTok{, }\StringTok{"vert"}\NormalTok{, }\StringTok{"rouge"}\NormalTok{, }\StringTok{"PIR"}\NormalTok{]}
\BuiltInTok{print}\NormalTok{(bandes[}\DecValTok{0}\NormalTok{])          }\CommentTok{\# premier élément}
\BuiltInTok{print}\NormalTok{(bandes[}\OperatorTok{{-}}\DecValTok{1}\NormalTok{])         }\CommentTok{\# dernier élément}
\BuiltInTok{print}\NormalTok{(bandes[}\DecValTok{1}\NormalTok{:}\DecValTok{3}\NormalTok{])        }\CommentTok{\# tranche (slice) : [\textquotesingle{}vert\textquotesingle{}, \textquotesingle{}rouge\textquotesingle{}]}

\NormalTok{bandes.append(}\StringTok{"SWIR"}\NormalTok{)     }\CommentTok{\# ajout en fin de liste}
\BuiltInTok{print}\NormalTok{(}\BuiltInTok{len}\NormalTok{(bandes), }\StringTok{"bandes :"}\NormalTok{, bandes)}

\CommentTok{\# Compréhension de liste : transformer chaque élément}
\BuiltInTok{print}\NormalTok{([b.upper() }\ControlFlowTok{for}\NormalTok{ b }\KeywordTok{in}\NormalTok{ bandes])}
\end{Highlighting}
\end{Shaded}
...
PIR
['vert', 'rouge']
5 bandes : ['bleu', 'vert', 'rouge', 'PIR', 'SWIR']
['BLEU', 'VERT', 'ROUGE', 'PIR', 'SWIR']
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Les dimensions (lignes, colonnes) d\textquotesingle{}une image : une donnée qui ne change pas}
\NormalTok{dimensions }\OperatorTok{=}\NormalTok{ (}\DecValTok{512}\NormalTok{, }\DecValTok{512}\NormalTok{)}
\NormalTok{lignes, colonnes }\OperatorTok{=}\NormalTok{ dimensions        }\CommentTok{\# dépaquetage (unpacking)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Lignes :"}\NormalTok{, lignes, }\StringTok{"| Colonnes :"}\NormalTok{, colonnes)}

\CommentTok{\# dimensions[0] = 1024   \# {-}\textgreater{} TypeError : un tuple est immuable}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Lignes : 512 | Colonnes : 512
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Éliminer les doublons d\textquotesingle{}une liste de classes d\textquotesingle{}occupation du sol}
\NormalTok{classes }\OperatorTok{=}\NormalTok{ [}\StringTok{"eau"}\NormalTok{, }\StringTok{"forêt"}\NormalTok{, }\StringTok{"eau"}\NormalTok{, }\StringTok{"urbain"}\NormalTok{, }\StringTok{"forêt"}\NormalTok{]}
\NormalTok{uniques }\OperatorTok{=} \BuiltInTok{set}\NormalTok{(classes)}
\BuiltInTok{print}\NormalTok{(uniques)}

\CommentTok{\# Opérations ensemblistes}
\NormalTok{a, b }\OperatorTok{=}\NormalTok{ \{}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{3}\NormalTok{\}, \{}\DecValTok{3}\NormalTok{, }\DecValTok{4}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(}\StringTok{"intersection :"}\NormalTok{, a }\OperatorTok{\&}\NormalTok{ b, }\StringTok{"| union :"}\NormalTok{, a }\OperatorTok{|}\NormalTok{ b)}
\end{Highlighting}
\end{Shaded}
...
Les dictionnaires sont comme des boites avec des étiquettes sur chacune
d'elles :
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Un dictionnaire : les métadonnées d\textquotesingle{}une image satellite}
\NormalTok{image }\OperatorTok{=}\NormalTok{ \{}\StringTok{"capteur"}\NormalTok{: }\StringTok{"Sentinel{-}2"}\NormalTok{, }\StringTok{"bandes"}\NormalTok{: }\DecValTok{13}\NormalTok{, }\StringTok{"resolution\_m"}\NormalTok{: }\DecValTok{10}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(image[}\StringTok{"capteur"}\NormalTok{])              }\CommentTok{\# accès par clé}

\NormalTok{image[}\StringTok{"date"}\NormalTok{] }\OperatorTok{=} \StringTok{"2024{-}07{-}01"}         \CommentTok{\# ajout d\textquotesingle{}une paire clé{-}valeur}
\ControlFlowTok{for}\NormalTok{ cle, valeur }\KeywordTok{in}\NormalTok{ image.items():    }\CommentTok{\# parcours des paires clé{-}valeur}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{cle}\SpecialCharTok{\}}\SpecialStringTok{ : }\SpecialCharTok{\{}\NormalTok{valeur}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\section{Boucles et conditions}\label{boucles-et-conditions}

Un programme prend des décisions (\texttt{if}) et répète des opérations
(\texttt{for}, \texttt{while}). Ces structures de contrôle sont au cœur
de tout traitement automatisé.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bandes }\OperatorTok{=}\NormalTok{ [}\StringTok{"bleu"}\NormalTok{, }\StringTok{"vert"}\NormalTok{, }\StringTok{"rouge"}\NormalTok{, }\StringTok{"PIR"}\NormalTok{]}

\CommentTok{\# Boucle for : parcourir chaque bande avec son indice}
\ControlFlowTok{for}\NormalTok{ i, nom }\KeywordTok{in} \BuiltInTok{enumerate}\NormalTok{(bandes):}
    \BuiltInTok{print}\NormalTok{(i, nom)}

\CommentTok{\# Condition if / elif / else}
\NormalTok{reflectance }\OperatorTok{=} \FloatTok{0.42}
\ControlFlowTok{if}\NormalTok{ reflectance }\OperatorTok{\textgreater{}} \FloatTok{0.5}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"forte réflectance"}\NormalTok{)}
\ControlFlowTok{elif}\NormalTok{ reflectance }\OperatorTok{\textgreater{}} \FloatTok{0.3}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"réflectance moyenne"}\NormalTok{)}
\ControlFlowTok{else}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"faible réflectance"}\NormalTok{)}

\CommentTok{\# Boucle while : tant qu\textquotesingle{}une condition est vraie}
\NormalTok{seuil, valeur }\OperatorTok{=} \FloatTok{0.5}\NormalTok{, }\FloatTok{0.1}
\ControlFlowTok{while}\NormalTok{ valeur }\OperatorTok{\textless{}}\NormalTok{ seuil:}
\NormalTok{    valeur }\OperatorTok{+=} \FloatTok{0.2}
\BuiltInTok{print}\NormalTok{(}\StringTok{"valeur finale :"}\NormalTok{, }\BuiltInTok{round}\NormalTok{(valeur, }\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Une fonction regroupe des instructions réutilisables sous un nom. On la
définit avec \texttt{def} ; elle reçoit des \emph{arguments} et renvoie
un résultat avec \texttt{return}.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ ndvi(nir, rouge):}
    \CommentTok{"""Indice de végétation NDVI = (PIR {-} Rouge) / (PIR + Rouge)."""}
    \ControlFlowTok{return}\NormalTok{ (nir }\OperatorTok{{-}}\NormalTok{ rouge) }\OperatorTok{/}\NormalTok{ (nir }\OperatorTok{+}\NormalTok{ rouge)}

\BuiltInTok{print}\NormalTok{(}\BuiltInTok{round}\NormalTok{(ndvi(}\FloatTok{0.6}\NormalTok{, }\FloatTok{0.2}\NormalTok{), }\DecValTok{3}\NormalTok{))}

\CommentTok{\# Argument par défaut}
\KeywordTok{def}\NormalTok{ normaliser(valeur, maximum}\OperatorTok{=}\DecValTok{255}\NormalTok{):}
    \ControlFlowTok{return}\NormalTok{ valeur }\OperatorTok{/}\NormalTok{ maximum}

\BuiltInTok{print}\NormalTok{(normaliser(}\DecValTok{128}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(normaliser(}\DecValTok{1000}\NormalTok{, maximum}\OperatorTok{=}\DecValTok{4095}\NormalTok{))   }\CommentTok{\# image 12 bits}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{class}\NormalTok{ Image:}
    \CommentTok{"""Une classe minimale décrivant une image satellite."""}

    \KeywordTok{def} \FunctionTok{\_\_init\_\_}\NormalTok{(}\VariableTok{self}\NormalTok{, capteur, bandes):   }\CommentTok{\# constructeur}
        \VariableTok{self}\NormalTok{.capteur }\OperatorTok{=}\NormalTok{ capteur              }\CommentTok{\# attributs}
        \VariableTok{self}\NormalTok{.bandes }\OperatorTok{=}\NormalTok{ bandes}

    \KeywordTok{def}\NormalTok{ resume(}\VariableTok{self}\NormalTok{):                       }\CommentTok{\# méthode}
        \ControlFlowTok{return} \SpecialStringTok{f"}\SpecialCharTok{\{}\VariableTok{self}\SpecialCharTok{.}\NormalTok{capteur}\SpecialCharTok{\}}\SpecialStringTok{ — }\SpecialCharTok{\{}\VariableTok{self}\SpecialCharTok{.}\NormalTok{bandes}\SpecialCharTok{\}}\SpecialStringTok{ bandes"}

\NormalTok{img }\OperatorTok{=}\NormalTok{ Image(}\StringTok{"Landsat{-}8"}\NormalTok{, }\DecValTok{11}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(img.resume())}
\BuiltInTok{print}\NormalTok{(img.capteur)}
\end{Highlighting}
\end{Shaded}
...
\section{Importer des
bibliothèques}\label{importer-des-bibliothuxe8ques}

Python possède une petite bibliothèque standard, mais toute sa puissance
vient des \emph{packages} externes (comme NumPy). On les installe une
fois avec \texttt{pip}, puis on les charge dans un script avec
\texttt{import}.

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{pip}\NormalTok{ install numpy          }\CommentTok{\# une seule fois par environnement}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ numpy }\ImportTok{as}\NormalTok{ np              }\CommentTok{\# tout le module, sous l\textquotesingle{}alias np}
\ImportTok{from}\NormalTok{ math }\ImportTok{import}\NormalTok{ pi, sqrt       }\CommentTok{\# seulement certains éléments}

\BuiltInTok{print}\NormalTok{(np.array([}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{3}\NormalTok{]))}
\BuiltInTok{print}\NormalTok{(}\BuiltInTok{round}\NormalTok{(pi, }\DecValTok{4}\NormalTok{), sqrt(}\DecValTok{16}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\section{Un avant-goût de NumPy}\label{sec-00-02}

Dans ce manuel, une image est avant tout un tableau de nombres. La
bibliothèque \href{https://numpy.org/}{NumPy} fournit l'objet
\texttt{ndarray} qui représente efficacement ces tableaux à plusieurs
dimensions : c'est la brique de base de tous les chapitres suivants.

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ numpy }\ImportTok{as}\NormalTok{ np}

\CommentTok{\# Une petite image à une bande : 3 lignes x 4 colonnes}
\NormalTok{image }\OperatorTok{=}\NormalTok{ np.array([[}\DecValTok{10}\NormalTok{, }\DecValTok{12}\NormalTok{, }\DecValTok{11}\NormalTok{,  }\DecValTok{9}\NormalTok{],}
\NormalTok{                  [ }\DecValTok{8}\NormalTok{, }\DecValTok{20}\NormalTok{, }\DecValTok{22}\NormalTok{,  }\DecValTok{7}\NormalTok{],}
\NormalTok{                  [ }\DecValTok{9}\NormalTok{, }\DecValTok{21}\NormalTok{, }\DecValTok{23}\NormalTok{,  }\DecValTok{8}\NormalTok{]])}

\BuiltInTok{print}\NormalTok{(}\StringTok{"Forme (lignes, colonnes) :"}\NormalTok{, image.shape)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Valeur maximale :"}\NormalTok{, image.}\BuiltInTok{max}\NormalTok{())}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Moyenne :"}\NormalTok{, image.mean().}\BuiltInTok{round}\NormalTok{(}\DecValTok{2}\NormalTok{))}

\CommentTok{\# Découpage d\textquotesingle{}une sous{-}image (2 premières lignes, 2 premières colonnes)}
\BuiltInTok{print}\NormalTok{(image[:}\DecValTok{2}\NormalTok{, :}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Forme (lignes, colonnes) : (3, 4)
Valeur maximale : 23
Moyenne : 13.33
[[10 12]
 [ 8 20]]
...
\section{Points clés}\label{points-cluxe9s}
...
\tightlist
\item
  Installez Python via un \textbf{environnement virtuel} (\texttt{conda}
  ou \texttt{venv}) pour isoler chaque projet.
\item
  Quatre structures de base : \textbf{listes} (ordonnées, modifiables),
  \textbf{tuples} (immuables), \textbf{ensembles} (sans doublons) et
  \textbf{dictionnaires} (paires clé-valeur).
\item
  \texttt{if}/\texttt{for}/\texttt{while} contrôlent le déroulement du
  programme ; les \textbf{fonctions} (\texttt{def}) regroupent du code
  réutilisable.
\item
  Une image est avant tout un \textbf{tableau \texttt{NumPy}} : c'est la
  structure centrale de tout le manuel.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Créez un dictionnaire \texttt{metadonnees} décrivant une image
  (capteur, nombre de bandes, résolution) puis affichez chaque paire
  clé-valeur.
\item
  À partir du tableau \texttt{image} de la section~\ref{sec-00-02},
  calculez la valeur \textbf{minimale} et l'\textbf{écart-type}
  (\texttt{image.std()}), puis extrayez la dernière colonne.
\item
  Écrivez une fonction \texttt{ratio(a,\ b)} renvoyant
  \texttt{(a\ -\ b)\ /\ (a\ +\ b)}, et utilisez-la pour un NDVI avec PIR
  = 0,55 et Rouge = 0,18.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1. Dictionnaire de métadonnées}
\NormalTok{metadonnees }\OperatorTok{=}\NormalTok{ \{}\StringTok{"capteur"}\NormalTok{: }\StringTok{"Sentinel{-}2"}\NormalTok{, }\StringTok{"bandes"}\NormalTok{: }\DecValTok{13}\NormalTok{, }\StringTok{"resolution\_m"}\NormalTok{: }\DecValTok{10}\NormalTok{\}}
\ControlFlowTok{for}\NormalTok{ cle, valeur }\KeywordTok{in}\NormalTok{ metadonnees.items():}
    \BuiltInTok{print}\NormalTok{(cle, }\StringTok{":"}\NormalTok{, valeur)}

\CommentTok{\# 2. Statistiques et dernière colonne du tableau \textasciigrave{}image\textasciigrave{}}
\BuiltInTok{print}\NormalTok{(}\StringTok{"min :"}\NormalTok{, image.}\BuiltInTok{min}\NormalTok{(), }\StringTok{"| écart{-}type :"}\NormalTok{, }\BuiltInTok{round}\NormalTok{(image.std(), }\DecValTok{2}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(}\StringTok{"dernière colonne :"}\NormalTok{, image[:, }\OperatorTok{{-}}\DecValTok{1}\NormalTok{])}

\CommentTok{\# 3. Fonction ratio et calcul du NDVI}
\KeywordTok{def}\NormalTok{ ratio(a, b):}
    \ControlFlowTok{return}\NormalTok{ (a }\OperatorTok{{-}}\NormalTok{ b) }\OperatorTok{/}\NormalTok{ (a }\OperatorTok{+}\NormalTok{ b)}

\BuiltInTok{print}\NormalTok{(}\StringTok{"NDVI :"}\NormalTok{, }\BuiltInTok{round}\NormalTok{(ratio(}\FloatTok{0.55}\NormalTok{, }\FloatTok{0.18}\NormalTok{), }\DecValTok{3}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{itemize}
\item
  \textbf{Quelle structure de données Python n'autorise }pas** les
  doublons ?**
  ...
  Relisez au besoin la section « Les ensembles (Sets) ».
\item
  \textbf{Les tuples peuvent être modifiés après leur création.}
  ...
  Relisez au besoin la section « Les tuples » : un tuple est immuable.
\item
  \textbf{Comment accède-t-on à la valeur associée à la clé
  \texttt{"capteur"} d'un dictionnaire nommé \texttt{image} ?}
  ...
  Relisez au besoin la section « Dictionnaires ».
\item
  \textbf{Parmi les affirmations suivantes sur les listes Python,
  ...
  \item
    Elles sont modifiables (\emph{mutable}).
  \item
    Elles autorisent les doublons.
  \item
    Elles sont délimitées par des accolades \texttt{\{\}}.
  \end{itemize}

  Relisez au besoin la section « Les listes ».
\item
  \textbf{Quel mot-clé permet de définir une fonction en Python ?}
  ...
  Relisez au besoin la section « Les fonctions ».
\item
  \textbf{Que renvoie l'expression \texttt{{[}1,\ 2,\ 3{]}{[}-1{]}} ?}
  ...
  L'indice -1 désigne le dernier élément. Voir la section « Les listes
  ».
\item
  \textbf{Pour un tableau NumPy \texttt{image}, quel attribut donne ses
  ...
  Relisez au besoin la section~\ref{sec-00-02}.
\item
  \textbf{Pourquoi utiliser un environnement virtuel (\texttt{conda} ou
  \texttt{venv}) ?}

  \begin{itemize}
  \tightlist
  \item
    Pour isoler les bibliothèques de chaque projet.
  \item
    Pour éviter les conflits entre versions de bibliothèques.
  \item
    Pour accélérer l'exécution du code Python.
  \item
    Pour remplacer l'interpréteur Python.
  \end{itemize}

  Relisez au besoin la section~\ref{sec-00-01}.
\end{itemize}
...
\tightlist
\item
  Quelle structure de données Python n'autorise \textbf{pas} les
  doublons ?
  ...
  \end{itemize}
\item
  Les tuples peuvent être modifiés après leur création.
  ...
  \end{itemize}
\item
  Comment accède-t-on à la valeur associée à la clé \texttt{"capteur"}
  d'un dictionnaire nommé \texttt{image} ?
  ...
  \end{itemize}
\item
  Parmi les affirmations suivantes sur les listes Python, lesquelles
  sont vraies ?
...
\item
  ...
  \end{itemize}
\item
  Que renvoie l'expression \texttt{{[}1,\ 2,\ 3{]}{[}-1{]}} ?
  ...
  \end{itemize}
\item
  Pour un tableau NumPy \texttt{image}, quel attribut donne ses
  dimensions (lignes, colonnes) ?
  ...
  \end{itemize}
\item
  Pourquoi utiliser un environnement virtuel (\texttt{conda} ou
  \texttt{venv}) ?
...
\item
  manipuler la matrice de la donnée d'une image avec \texttt{numpy};
\item
  exploiter les métadonnées et la géoréférence avec \texttt{rioxarray}
  et \texttt{xarray}.
\end{itemize}
...
\OperatorTok{!}\NormalTok{pip install }\OperatorTok{{-}}\NormalTok{q rioxarray xrscipy}
\end{Highlighting}
\end{Shaded}
...
géomatique qui permet d'importer (et d'exporter) de l'imagerie est la
librairie \href{https://gdal.org}{GDAL} qui rassemble la plupart des
formats sous forme de \emph{driver} (ou pilote en français).

Dans le domaine de la géomatique, il faut prêter attention à quatre
...
librairie \href{https://numpy.org/}{NumPy} (voir la section «
Manipulation de la matrice de pixels » plus bas).
...
ces quatre caractéristiques sont gérées.
...
comme OpenCV et scikit-image ainsi que les grandes bibliothèques en
...
Attention : la fonction \texttt{imread} d'OpenCV renvoie les canaux dans
l'ordre \textbf{BGR} (bleu, vert, rouge) et non RGB. Il faut convertir
l'image avant de l'afficher correctement avec \texttt{matplotlib} :

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ matplotlib.pyplot }\ImportTok{as}\NormalTok{ plt}

\NormalTok{img\_bgr }\OperatorTok{=}\NormalTok{ cv2.imread(}\StringTok{\textquotesingle{}modis{-}aqua.PNG\textquotesingle{}}\NormalTok{)}
\NormalTok{img\_rgb }\OperatorTok{=}\NormalTok{ cv2.cvtColor(img\_bgr, cv2.COLOR\_BGR2RGB)   }\CommentTok{\# BGR {-}\textgreater{} RGB}
...
\NormalTok{ax[}\DecValTok{0}\NormalTok{].imshow(img\_bgr)}\OperatorTok{;}\NormalTok{ ax[}\DecValTok{0}\NormalTok{].set\_title(}\StringTok{"Sans conversion (BGR)"}\NormalTok{)}\OperatorTok{;}\NormalTok{ ax[}\DecValTok{0}\NormalTok{].axis(}\StringTok{\textquotesingle{}off\textquotesingle{}}\NormalTok{)}
\NormalTok{ax[}\DecValTok{1}\NormalTok{].imshow(img\_rgb)}\OperatorTok{;}\NormalTok{ ax[}\DecValTok{1}\NormalTok{].set\_title(}\StringTok{"Après conversion (RGB)"}\NormalTok{)}\OperatorTok{;}\NormalTok{ ax[}\DecValTok{1}\NormalTok{].axis(}\StringTok{\textquotesingle{}off\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.show()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-ImportationManipulationImages_files/figure-pdf/cell-8-output-1.pdf}}
...
\begin{verbatim}
/usr/bin/sh: 1: gdalinfo: not found
\end{verbatim}
...
Depuis Python, \texttt{rasterio} donne accès à ces mêmes informations
sous forme d'attributs et d'un dictionnaire de métadonnées :
...
\ControlFlowTok{with}\NormalTok{ rasterio.}\BuiltInTok{open}\NormalTok{(}\StringTok{\textquotesingle{}RGBNIR\_of\_S2A.tif\textquotesingle{}}\NormalTok{) }\ImportTok{as}\NormalTok{ src:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"Nombre de bandes :"}\NormalTok{, src.count)}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"Dimensions (lignes, colonnes) :"}\NormalTok{, src.height, src.width)}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"Type des pixels :"}\NormalTok{, src.dtypes[}\DecValTok{0}\NormalTok{])}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"Système de coordonnées :"}\NormalTok{, src.crs)}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"Résolution (m) :"}\NormalTok{, src.res)}
    \BuiltInTok{print}\NormalTok{(}\StringTok{"Emprise :"}\NormalTok{, src.bounds)}
\NormalTok{    meta }\OperatorTok{=}\NormalTok{ src.meta            }\CommentTok{\# toutes les métadonnées dans un dictionnaire}
\NormalTok{meta}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
Nombre de bandes : 4
Dimensions (lignes, colonnes) : 1926 2074
Type des pixels : uint16
Système de coordonnées : EPSG:32618
Résolution (m) : (10.0, 10.0)
Emprise : BoundingBox(left=731780.0, bottom=5021540.0, right=752520.0, top=5040800.0)
\end{verbatim}
...
{'driver': 'GTiff',
 ...
 'transform': Affine(10.0, 0.0, 731780.0,
        0.0, -10.0, 5040800.0)}
\end{verbatim}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2537}}
...
en Python. Les indices commencent à \texttt{0} et se terminent à la
...
\item
  ...
  \texttt{matrice{[}1,\ 1,\ :{]}};
\item
  choisir la troisième colonne (indice 2) avec toutes les bandes :
  \texttt{matrice{[}:,\ 2,\ :{]}}.
\end{itemize}
...
\ImportTok{import}\NormalTok{ cv2}
\NormalTok{img }\OperatorTok{=}\NormalTok{ cv2.imread(}\StringTok{\textquotesingle{}modis{-}aqua.PNG\textquotesingle{}}\NormalTok{)}
\NormalTok{img\_col }\OperatorTok{=}\NormalTok{ img[:,}\DecValTok{2}\NormalTok{,:]   }\CommentTok{\# troisième colonne (indice 2), toutes les bandes}
...
\subsection{Masquage}\label{masquage}
...
certaines valeurs. Le module \texttt{numpy.ma} permet de créer un
tableau masqué où les pixels non valides sont ignorés par les calculs
statistiques :
...
\CommentTok{\# Une petite image dont la valeur 0 représente le "no data"}
\NormalTok{img }\OperatorTok{=}\NormalTok{ np.array([[}\DecValTok{12}\NormalTok{,  }\DecValTok{0}\NormalTok{, }\DecValTok{15}\NormalTok{],}
\NormalTok{                [ }\DecValTok{8}\NormalTok{, }\DecValTok{20}\NormalTok{,  }\DecValTok{0}\NormalTok{],}
\NormalTok{                [ }\DecValTok{0}\NormalTok{, }\DecValTok{21}\NormalTok{, }\DecValTok{23}\NormalTok{]], dtype}\OperatorTok{=}\BuiltInTok{float}\NormalTok{)}

\CommentTok{\# Masquer les pixels égaux à 0 : ils seront ignorés par les statistiques}
\NormalTok{img\_masque }\OperatorTok{=}\NormalTok{ np.ma.masked\_equal(img, }\DecValTok{0}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Moyenne sans masque :"}\NormalTok{, }\BuiltInTok{round}\NormalTok{(img.mean(), }\DecValTok{2}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Moyenne avec masque :"}\NormalTok{, }\BuiltInTok{round}\NormalTok{(img\_masque.mean(), }\DecValTok{2}\NormalTok{))}

\CommentTok{\# Approche équivalente avec des NaN et les fonctions « nan » de NumPy}
\NormalTok{img\_nan }\OperatorTok{=}\NormalTok{ np.where(img }\OperatorTok{==} \DecValTok{0}\NormalTok{, np.nan, img)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Moyenne (nanmean)   :"}\NormalTok{, }\BuiltInTok{round}\NormalTok{(np.nanmean(img\_nan), }\DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Moyenne sans masque : 11.0
Moyenne avec masque : 16.5
Moyenne (nanmean)   : 16.5
\end{verbatim}

\subsection{Calcul d'un rapport de
bandes}\label{calcul-dun-rapport-de-bandes}

Une opération très courante consiste à combiner deux bandes pixel par
pixel. Par exemple, un rapport normalisé entre le proche-infrarouge et
le rouge met en évidence la végétation (ce type d'indice spectral est
approfondi au chapitre Chapitre~\ref{sec-chap03}) :

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ rioxarray }\ImportTok{as}\NormalTok{ rxr}

\NormalTok{img }\OperatorTok{=}\NormalTok{ rxr.open\_rasterio(}\StringTok{\textquotesingle{}RGBNIR\_of\_S2A.tif\textquotesingle{}}\NormalTok{).astype(}\StringTok{\textquotesingle{}float32\textquotesingle{}}\NormalTok{)}
\NormalTok{rouge }\OperatorTok{=}\NormalTok{ img.sel(band}\OperatorTok{=}\DecValTok{1}\NormalTok{)     }\CommentTok{\# bande 1 : rouge}
\NormalTok{pir   }\OperatorTok{=}\NormalTok{ img.sel(band}\OperatorTok{=}\DecValTok{4}\NormalTok{)     }\CommentTok{\# bande 4 : proche{-}infrarouge}

\CommentTok{\# Rapport normalisé (NIR {-} Rouge) / (NIR + Rouge)}
\NormalTok{rapport }\OperatorTok{=}\NormalTok{ (pir }\OperatorTok{{-}}\NormalTok{ rouge) }\OperatorTok{/}\NormalTok{ (pir }\OperatorTok{+}\NormalTok{ rouge)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Forme du rapport :"}\NormalTok{, rapport.shape)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Valeurs min/max  :"}\NormalTok{, }\BuiltInTok{round}\NormalTok{(}\BuiltInTok{float}\NormalTok{(rapport.}\BuiltInTok{min}\NormalTok{()), }\DecValTok{2}\NormalTok{), }\BuiltInTok{round}\NormalTok{(}\BuiltInTok{float}\NormalTok{(rapport.}\BuiltInTok{max}\NormalTok{()), }\DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Forme du rapport : (1926, 2074)
Valeurs min/max  : -0.42 0.93
\end{verbatim}

\subsection{Exportation d'une image}\label{exportation-dune-image}

Une image chargée avec \texttt{rioxarray} peut être réécrite sur le
disque au format GeoTIFF avec \texttt{rio.to\_raster}, en conservant sa
géoréférence :
...
\NormalTok{img }\OperatorTok{=}\NormalTok{ rxr.open\_rasterio(}\StringTok{\textquotesingle{}RGBNIR\_of\_S2A.tif\textquotesingle{}}\NormalTok{)}
\CommentTok{\# On ne conserve que les trois premières bandes (rouge, vert, bleu)}
\NormalTok{img\_rvb }\OperatorTok{=}\NormalTok{ img.sel(band}\OperatorTok{=}\NormalTok{[}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{3}\NormalTok{])}
\NormalTok{img\_rvb.rio.to\_raster(}\StringTok{\textquotesingle{}RGBNIR\_rvb.tif\textquotesingle{}}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Image exportée :"}\NormalTok{, }\BuiltInTok{tuple}\NormalTok{(img\_rvb.sizes.values()))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Image exportée : (3, 1926, 2074)
\end{verbatim}

\subsection{Changement de projection
cartographique}\label{changement-de-projection-cartographique}

Une image géoréférencée peut être reprojetée dans un autre système de
coordonnées avec \texttt{rio.reproject}. \texttt{rioxarray} recalcule
alors la grille de pixels et met à jour la géoréférence :
...
\NormalTok{img }\OperatorTok{=}\NormalTok{ rxr.open\_rasterio(}\StringTok{\textquotesingle{}RGBNIR\_of\_S2A.tif\textquotesingle{}}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"CRS d\textquotesingle{}origine :"}\NormalTok{, img.rio.crs)              }\CommentTok{\# EPSG:32618 (UTM)}

\CommentTok{\# Reprojection en coordonnées géographiques (latitude/longitude)}
\NormalTok{img\_wgs84 }\OperatorTok{=}\NormalTok{ img.rio.reproject(}\StringTok{"EPSG:4326"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Après reprojection :"}\NormalTok{, img\_wgs84.rio.crs)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Nouvelle forme :"}\NormalTok{, }\BuiltInTok{dict}\NormalTok{(img\_wgs84.sizes))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
CRS d'origine : EPSG:32618
\end{verbatim}

\begin{verbatim}
Après reprojection : EPSG:4326
Nouvelle forme : {'band': 4, 'y': 1633, 'x': 2483}
\end{verbatim}
...
Ces deux structures prennent en charge les opérations de type
...
\subsubsection{Exemple avec rioxarray}\label{exemple-avec-rioxarray}

\texttt{rioxarray} ouvre un GeoTIFF directement comme un
\texttt{DataArray} étiqueté : ses dimensions (\texttt{band}, \texttt{y},
\texttt{x}), ses coordonnées et sa géoréférence sont accessibles via
l'accesseur \texttt{.rio}. On peut alors sélectionner une bande par son
étiquette plutôt que par un numéro d'axe :
...
\NormalTok{img }\OperatorTok{=}\NormalTok{ rxr.open\_rasterio(}\StringTok{\textquotesingle{}RGBNIR\_of\_S2A.tif\textquotesingle{}}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Dimensions :"}\NormalTok{, }\BuiltInTok{dict}\NormalTok{(img.sizes))}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Système de coordonnées :"}\NormalTok{, img.rio.crs)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Résolution (m) :"}\NormalTok{, img.rio.resolution())}

\CommentTok{\# Sélection de la bande proche{-}infrarouge (bande 4) par son étiquette}
\NormalTok{pir }\OperatorTok{=}\NormalTok{ img.sel(band}\OperatorTok{=}\DecValTok{4}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Bande PIR — forme :"}\NormalTok{, pir.shape, }\StringTok{"| valeur maximale :"}\NormalTok{, }\BuiltInTok{int}\NormalTok{(pir.}\BuiltInTok{max}\NormalTok{()))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Dimensions : {'band': 4, 'y': 1926, 'x': 2074}
Système de coordonnées : EPSG:32618
Résolution (m) : (10.0, -10.0)
Bande PIR — forme : (1926, 2074) | valeur maximale : 15642
\end{verbatim}

\section{Points clés}\label{points-cluxe9s-1}
...
\tightlist
\item
  \textbf{GDAL} est la librairie de référence pour lire/écrire
  l'imagerie (elle rassemble les formats sous forme de \emph{drivers}).
\item
  Deux familles de formats : \textbf{RVB} (JPEG, PNG --- ≤ 3 bandes, 8
  bits, peu de métadonnées) et \textbf{géo-formats} (GeoTIFF, NetCDF,
  HDF5 --- multi-bandes, 16 bits/float, géoréférence). Le \textbf{COG}
  permet l'accès partiel à distance.
\item
  Quatre caractéristiques à surveiller : la \textbf{matrice de pixels},
  la \textbf{dynamique} (\texttt{dtype}), le \textbf{nombre de bandes}
  et les \textbf{métadonnées / géoréférence}.
\item
  Consulter les \textbf{métadonnées avant de lire} (\texttt{rio\ info},
  \texttt{gdalinfo}, ou les attributs de \texttt{rasterio}).
\item
  \texttt{OpenCV} lit les canaux en \textbf{BGR} : convertir en RGB
  (\texttt{cv2.cvtColor}) avant l'affichage.
\item
  Une image est un \textbf{tableau NumPy} : indexation/découpage
  \texttt{{[}ligne,\ colonne,\ bande{]}} ; attention à la distinction
  \textbf{vue vs copie} (\texttt{np.shares\_memory}, \texttt{.copy()}).
\item
  \texttt{rioxarray}/\texttt{xarray} ajoutent \textbf{dimensions
  nommées, coordonnées et géoréférence} (accesseur \texttt{.rio}) :
  sélection par étiquette, reprojection (\texttt{rio.reproject}), export
  (\texttt{rio.to\_raster}).
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Ouvrez \texttt{RGBNIR\_of\_S2A.tif} avec \texttt{rasterio} et affichez
  son nombre de bandes, son type de pixels, son système de coordonnées
  et sa résolution.
\item
  À l'aide du découpage NumPy, extrayez une fenêtre de 100 × 100 pixels
  au centre d'une image et affichez-la.
\item
  Vérifiez avec \texttt{np.shares\_memory} si un découpage crée une
  \textbf{vue} ou une \textbf{copie}, puis forcez une copie avec
  \texttt{.copy()}.
\item
  Reprojetez \texttt{RGBNIR\_of\_S2A.tif} en \texttt{EPSG:4326} avec
  \texttt{rioxarray}, puis sauvegardez le résultat en GeoTIFF avec
  \texttt{rio.to\_raster}.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{Quelle bibliothèque rassemble la plupart des formats
  matriciels en géomatique et permet de les lire et de les écrire ?}
  ...
  Relisez au besoin la section « Importation d'images ».
\item
  \textbf{Les formats RVB grand public (JPEG, PNG) supportent
  généralement plus de trois bandes.}
  ...
  Relisez au besoin la section « Formats des images » : ces formats se
  limitent à trois bandes (RVB).
\item
  \textbf{Parmi les caractéristiques suivantes d'une image, lesquelles
  distinguent les différents formats ?}
  ...
  \item
    La dynamique (le type de stockage des valeurs)
  \item
    Le nombre de bandes
  \item
    La métadonnée (dont la géoréférence)
  \end{itemize}

  Relisez au besoin la section « Importation d'images » : quatre
  caractéristiques principales.
\item
  \textbf{Quelle est la plage de valeurs d'un pixel de type
  \texttt{uint8} ?}
  ...
  Relisez au besoin le tableau des types de données NumPy
  (tableau~\ref{tbl-numpytype}).
\item
  \textbf{Dans quel ordre la fonction \texttt{cv2.imread} d'OpenCV
  renvoie-t-elle les canaux ?}
  ...
  Relisez au besoin la section « Lecture avec la librairie OpenCV ».
\item
  \textbf{Pour une matrice NumPy \texttt{img}, quel attribut donne son
  type de stockage (par ex. \texttt{uint8}) ?}
  ...
  Relisez au besoin la section « Information de base ».
\item
  \textbf{Que sélectionne le découpage \texttt{matrice{[}:,\ 2,\ :{]}}
  ?}
  ...
  \item
    La deuxième ligne
  \item
    La troisième colonne (indice 2), toutes bandes comprises
  \item
    La bande numéro 2
  \item
    Un seul pixel
  ...
  Relisez au besoin la section « Découpage et indexation de la matrice »
  : les indices commencent à 0.
\item
  \textbf{Quels avantages \texttt{xarray} / \texttt{rioxarray}
  apportent-ils par rapport à un simple tableau NumPy ?}

  \begin{itemize}
  \tightlist
  \item
    Des dimensions nommées (band, y, x)
  \item
    La sélection par étiquette avec \texttt{.sel}
  \item
    L'accès à la géoréférence via l'accesseur \texttt{.rio}
  \item
    L'augmentation automatique de la résolution spatiale
  \end{itemize}

  Relisez au besoin la section « Données en géoscience ».
\item
  \textbf{Quel format est un GeoTIFF optimisé pour un accès partiel via
  HTTP (infonuagique) ?}
  ...
  Relisez au besoin la section « Le format COG ».
\end{itemize}
...
\tightlist
\item
  Quelle bibliothèque rassemble la plupart des formats matriciels en
  géomatique et permet de les lire et de les écrire ?
  ...
  \end{itemize}
\item
  Les formats RVB grand public (JPEG, PNG) supportent généralement plus
  de trois bandes.
  ...
  \end{itemize}
\item
  Parmi les caractéristiques suivantes d'une image, lesquelles
  distinguent les différents formats ?
  ...
  \end{itemize}
\item
  Quelle est la plage de valeurs d'un pixel de type \texttt{uint8} ?
  ...
  \end{itemize}
\item
  Dans quel ordre la fonction \texttt{cv2.imread} d'OpenCV
  renvoie-t-elle les canaux ?
  ...
  \end{itemize}
\item
  Pour une matrice NumPy \texttt{img}, quel attribut donne son type de
  stockage (par ex. \texttt{uint8}) ?
  ...
  \end{itemize}
\item
  Que sélectionne le découpage \texttt{matrice{[}:,\ 2,\ :{]}} ?
  ...
  \end{itemize}
\item
  Quels avantages \texttt{xarray} / \texttt{rioxarray} apportent-ils par
  rapport à un simple tableau NumPy ?
  ...
  \end{itemize}
\item
  Quel format est un GeoTIFF optimisé pour un accès partiel via HTTP
  (infonuagique) ?
...
D'emblée, il faut mentionner que Python n'est pas vraiment fait pour
...
\ImportTok{import}\NormalTok{ matplotlib.pyplot }\ImportTok{as}\NormalTok{ plt}
\NormalTok{fig, ax}\OperatorTok{=}\NormalTok{ plt.subplots(figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{6}\NormalTok{, }\DecValTok{5}\NormalTok{))}
\CommentTok{\# on garde les 3 premières bandes (R,V,B) : une 4e bande serait lue comme canal alpha}
\NormalTok{plt.imshow(img\_rgbnir.data[}\DecValTok{0}\NormalTok{:}\DecValTok{3}\NormalTok{].transpose(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{0}\NormalTok{)}\OperatorTok{/}\FloatTok{2500.0}\NormalTok{)}
\NormalTok{plt.show()}
\end{Highlighting}
\end{Shaded}
...
\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}limites :\textquotesingle{}}\NormalTok{,bin\_limites)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
valeurs : [0.15555556 0.11111111 0.16666667 0.15555556 0.04444444 0.05555556
 0.11111111 0.1        0.05555556 0.15555556]
...
\begin{equation}\protect\phantomsection\label{eq-rehauss-lin}{ \text{nouvelle valeur d'un pixel} = \frac{\text{valeur d'un pixel} - min_0}{max_0 - min_0}\times (max_1 - min_1)+min_1}\end{equation}
...
\begin{Highlighting}[]
\NormalTok{percentiles\_position}\OperatorTok{=}\NormalTok{ (}\DecValTok{0}\NormalTok{,}\FloatTok{0.1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{50}\NormalTok{,}\DecValTok{98}\NormalTok{,}\DecValTok{99}\NormalTok{,}\FloatTok{99.9}\NormalTok{,}\DecValTok{100}\NormalTok{)}
\NormalTok{sar}\OperatorTok{=}\NormalTok{ img\_SAR[}\DecValTok{0}\NormalTok{].data}
\CommentTok{\# on masque le no\_data et les valeurs \textless{}= 0 (impropres au log) : sinon 10*log10 {-}\textgreater{} {-}inf/nan}
\NormalTok{valid}\OperatorTok{=}\NormalTok{ (sar }\OperatorTok{!=}\NormalTok{ NO\_DATA\_FLOAT) }\OperatorTok{\&}\NormalTok{ (sar }\OperatorTok{\textgreater{}} \DecValTok{0}\NormalTok{)}
\NormalTok{values}\OperatorTok{=} \DecValTok{10}\OperatorTok{*}\NormalTok{np.log10(np.where(valid, sar, np.nan))  }\CommentTok{\# image en dB, nan hors du masque}
\NormalTok{percentiles\_db}\OperatorTok{=} \BuiltInTok{dict}\NormalTok{(}\BuiltInTok{zip}\NormalTok{(percentiles\_position, np.nanpercentile(values, percentiles\_position)))}
\BuiltInTok{print}\NormalTok{(percentiles\_db)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
{0: np.float64(-50.876407623291016), 0.1: np.float64(-47.98946910476685), 1: np.float64(-40.62594409942627), 2: np.float64(-37.24779266357422), 50: np.float64(-19.075312614440918), 98: np.float64(-7.6460519409179675), 99: np.float64(-5.534138813018874), 99.9: np.float64(1.8286541197300006), 100: np.float64(26.84148406982422)}
\end{verbatim}
...
\begin{Highlighting}[]
\NormalTok{sar}\OperatorTok{=}\NormalTok{ img\_SAR[}\DecValTok{0}\NormalTok{].data}
\CommentTok{\# masque : no\_data et valeurs \textless{}= 0 exclues avant le passage en dB}
\NormalTok{valid}\OperatorTok{=}\NormalTok{ (sar }\OperatorTok{!=}\NormalTok{ NO\_DATA\_FLOAT) }\OperatorTok{\&}\NormalTok{ (sar }\OperatorTok{\textgreater{}} \DecValTok{0}\NormalTok{)}
\NormalTok{sar\_db}\OperatorTok{=} \DecValTok{10}\OperatorTok{*}\NormalTok{np.log10(np.where(valid, sar, np.nan)) }\CommentTok{\# image en dB (nan hors masque)}
\NormalTok{values}\OperatorTok{=}\NormalTok{ np.sort(sar\_db[valid].flatten()) }\CommentTok{\# valeurs valides rangées par ordre croissant}
\NormalTok{cdf\_x}\OperatorTok{=}\NormalTok{ np.linspace(values[}\DecValTok{0}\NormalTok{], values[}\OperatorTok{{-}}\DecValTok{1}\NormalTok{], }\DecValTok{1000}\NormalTok{) }\CommentTok{\# 1000 valeurs réparties également sur l\textquotesingle{}intervalle des valeurs}
\NormalTok{cdf\_source}\OperatorTok{=}\NormalTok{ np.interp(cdf\_x, values, np.arange(}\BuiltInTok{len}\NormalTok{(values))}\OperatorTok{/}\BuiltInTok{len}\NormalTok{(values)}\OperatorTok{*}\DecValTok{255}\NormalTok{) }\CommentTok{\# calcul de la CDF source}
\CommentTok{\# on applique sur la MÊME échelle (dB) que celle utilisée pour construire la CDF}
\NormalTok{values\_eq}\OperatorTok{=}\NormalTok{ np.interp(sar\_db, cdf\_x, cdf\_source)}
\NormalTok{values\_eq}\OperatorTok{=}\NormalTok{ np.where(valid, values\_eq, }\DecValTok{0}\NormalTok{).astype(}\StringTok{\textquotesingle{}uint8\textquotesingle{}}\NormalTok{) }\CommentTok{\# no\_data remis à 0}
...
\section{Points clés}\label{points-cluxe9s-2}
...
\tightlist
\item
  Le réhaussement visuel améliore l'affichage (contraste, dynamique)
  \textbf{sans modifier la donnée} d'origine : il est appliqué
  dynamiquement à l'affichage.
\item
  Les \textbf{statistiques} d'une image (min/max, moyenne, quantiles,
  écart-type) guident le choix de la transformation.
\item
  Le \textbf{stretch linéaire} remappe la dynamique ; pour les
  histogrammes \textbf{asymétriques} (images SAR), les
  \textbf{percentiles} (p.~ex. 2 \%--98 \%) fournissent des bornes
  d'affichage robustes.
\item
  Les transformations \textbf{non linéaires} --- passage en \textbf{dB}
  (\texttt{10·log10}) et \textbf{égalisation d'histogramme} ---
  rétablissent une distribution exploitable.
\item
  Les \textbf{palettes de couleur} et les \textbf{composés colorés}
  (choix des bandes) révèlent des propriétés différentes de la surface ;
  \texttt{vmin}/\texttt{vmax} contrôlent la dynamique affichée.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Proposez une autre transformation non linéaire pour l'image SAR
  (p.~ex. la racine carrée ou \texttt{np.arcsinh}) et comparez son
  histogramme à celui obtenu en décibels.
\item
  Reprenez l'égalisation d'histogramme en remplaçant la CDF cible
  équiprobable par une \textbf{CDF gaussienne} (avec
  \texttt{scipy.stats.norm.cdf}), puis observez l'effet sur l'image SAR.
\item
  À partir de \texttt{img\_s2}, construisez un nouveau composé coloré
  (p.~ex. \texttt{{[}11,\ 8,\ 4{]}} ou \texttt{{[}8,\ 4,\ 3{]}}) et
  décrivez les surfaces qu'il met en valeur.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{Quelle est la principale bibliothèque de visualisation
  d'images en Python ?}
...
\item
  \textbf{Dans quel ordre \texttt{imshow} attend-il les dimensions d'une
  image à plusieurs bandes ?}

  \begin{itemize}
  \tightlist
  \item
    bande, hauteur, largeur
  \item
    hauteur, largeur, bande
  \item
  ...
  Relisez au besoin la section « Visualisation en Python ».
\item
  \textbf{Pour afficher une image à trois bandes avec \texttt{imshow},
  les valeurs doivent être :}

  \begin{itemize}
  \tightlist
  \item
    Ramenées entre 0 et 1 (ou entre 0 et 255)
  \item
    Laissées telles quelles
  \item
    Converties en entiers 64 bits
  \item
    Transformées en décibels
  \end{itemize}

  Relisez au besoin la section « Visualisation en Python ».
\item
  \textbf{Le réhaussement visuel modifie de façon permanente les valeurs
  de la donnée d'origine.}
  ...
  Le réhaussement est appliqué dynamiquement à l'affichage; relisez la
  section « Réhaussements visuels ».
\item
  \textbf{Quelles statistiques univariées sont utiles au réhaussement
  d'une image ?}
...
\item
  \textbf{Pour une distribution très asymétrique (p.~ex. une image SAR),
  quel outil permet de fixer des bornes d'affichage robustes ?}

  \begin{itemize}
  \tightlist
  \item
    La valeur moyenne seule
  \item
    Les percentiles (p.~ex. 2 \% et 98 \%)
  \item
    La valeur maximale seule
  \item
    Le nombre de bandes
  \end{itemize}
...
\item
  \textbf{Pour une image radar, quelle transformation non linéaire est
  populaire pour afficher la rétrodiffusion ?}

  \begin{itemize}
  \tightlist
  \item
    Le passage en décibels (10·log10)
  \item
    La mise au carré
  \item
    L'inverse (1/x)
...
\item
  \textbf{Quel est le but de l'égalisation d'histogramme ?}

  \begin{itemize}
  \tightlist
  \item
    Rendre les valeurs de pixels équiprobables (histogramme aplati)
  \item
    Réduire la résolution spatiale de l'image
  \item
    Supprimer le bruit thermique du capteur
  \item
    Géoréférencer l'image
  \end{itemize}

  Relisez au besoin la section « Égalisation d'histogramme ».
\item
  \textbf{Les paramètres \texttt{vmin} et \texttt{vmax} de
  \texttt{imshow} contrôlent la dynamique affichée sans modifier la
  donnée.}
  ...
  Relisez au besoin la section « Cas des histogrammes asymétriques ».
\item
  \textbf{Dans l'exemple du chapitre, quel triplet de bandes Sentinel-2
  produit un composé vraies couleurs (RVB) ?}
...
\tightlist
\item
  Quelle est la principale bibliothèque de visualisation d'images en
  Python ?
  ...
  \end{itemize}
\item
  Dans quel ordre \texttt{imshow} attend-il les dimensions d'une image à
  plusieurs bandes ?
  ...
  \end{itemize}
\item
  Pour afficher une image à trois bandes avec \texttt{imshow}, les
  ...
  \end{itemize}
\item
  Le réhaussement visuel modifie de façon permanente les valeurs de la
  donnée d'origine.
  ...
  \end{itemize}
\item
  Quelles statistiques univariées sont utiles au réhaussement d'une
  image ?
  ...
  \end{itemize}
\item
  Pour une distribution très asymétrique (p.~ex. une image SAR), quel
  outil permet de fixer des bornes d'affichage robustes ?
  ...
  \end{itemize}
\item
  Pour une image radar, quelle transformation non linéaire est populaire
  pour afficher la rétrodiffusion ?
...
\item
  ...
  \end{itemize}
\item
  Les paramètres \texttt{vmin} et \texttt{vmax} de \texttt{imshow}
  contrôlent la dynamique affichée sans modifier la donnée.
  ...
  \end{itemize}
\item
  Dans l'exemple du chapitre, quel triplet de bandes Sentinel-2 produit
  un composé vraies couleurs (RVB) ?
...
\pandocbounded{\includegraphics[keepaspectratio]{03-TransformationSpectrales_files/figure-pdf/cell-7-output-2.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sentinel2\_bands}\OperatorTok{=}\NormalTok{ [}\StringTok{"A"}\NormalTok{, }\StringTok{"B"}\NormalTok{, }\StringTok{"G"}\NormalTok{, }\StringTok{"R"}\NormalTok{, }\StringTok{"RE1"}\NormalTok{, }\StringTok{"RE2"}\NormalTok{, }\StringTok{"RE3"}\NormalTok{, }\StringTok{"N"}\NormalTok{, }\StringTok{"N2"}\NormalTok{, }\StringTok{"WV"}\NormalTok{, }\StringTok{"S1"}\NormalTok{, }\StringTok{"S2"}\NormalTok{]}
\NormalTok{img\_s2 }\OperatorTok{=}\NormalTok{ img\_s2.sel(band }\OperatorTok{=} \BuiltInTok{list}\NormalTok{(}\BuiltInTok{range}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{13}\NormalTok{))).assign\_coords(\{}\StringTok{\textquotesingle{}band\textquotesingle{}}\NormalTok{:sentinel2\_bands\})}
\NormalTok{img\_s2}\OperatorTok{=}\NormalTok{img\_s2}\OperatorTok{/}\DecValTok{10000} \CommentTok{\# normalisation en réflectance}
\end{Highlighting}
\end{Shaded}

Le \textbf{NDVI} (\emph{Normalized Difference Vegetation Index}) est
l'indice le plus connu. Il se calcule à partir des bandes rouge (\(R\))
et proche-infrarouge (\(N\)) :

\begin{equation}\protect\phantomsection\label{eq-ndvi}{ NDVI = \frac{N - R}{N + R} }\end{equation}

La végétation en bonne santé réfléchit fortement le proche-infrarouge et
absorbe le rouge : son NDVI est donc élevé (proche de \(1\)), alors que
l'eau, le sol nu ou le bâti donnent des valeurs faibles, voire
négatives. Le \texttt{GNDVI} remplace le rouge par le vert et le
\texttt{SAVI} ajoute un facteur de correction du sol (\(L\)). On calcule
ces trois indices ci-dessous avec \texttt{spyndex.computeIndex} :
...
\pandocbounded{\includegraphics[keepaspectratio]{03-TransformationSpectrales_files/figure-pdf/cell-13-output-1.pdf}}
...
\NormalTok{    params }\OperatorTok{=}\NormalTok{ \{}
        \StringTok{"N"}\NormalTok{: df[}\StringTok{"N"}\NormalTok{],}
        \StringTok{"R"}\NormalTok{: df[}\StringTok{"R"}\NormalTok{],}
        \StringTok{"G"}\NormalTok{: df[}\StringTok{"G"}\NormalTok{],}
        \StringTok{"S1"}\NormalTok{: df[}\StringTok{"S1"}\NormalTok{]}
\NormalTok{    \}}
\NormalTok{)}
\NormalTok{idx[}\StringTok{"Land Cover"}\NormalTok{] }\OperatorTok{=}\NormalTok{ [nom\_classes[l] }\ControlFlowTok{for}\NormalTok{ l }\KeywordTok{in}\NormalTok{ df[}\StringTok{"class"}\NormalTok{].tolist()] }\CommentTok{\# Add Land Cover to DataFrame}
...
\section{Points clés}\label{points-cluxe9s-3}
...
\tightlist
\item
  L'\textbf{information spectrale} exploite la dimension des bandes ; la
  \textbf{signature spectrale} caractérise le type et l'état d'un
  matériau --- une généralisation de la couleur au-delà du visible.
\item
  Le nombre de bandes distingue les capteurs : \textbf{multispectral}
  (une quinzaine) vs \textbf{hyperspectral} (des centaines).
\item
  Un \textbf{indice spectral} met en valeur des caractéristiques du
  spectre (pentes, gradients) et améliore la \textbf{séparabilité} des
  classes d'intérêt.
\item
  La librairie \textbf{\texttt{spyndex}} (Awesome Spectral Indices)
  donne accès à plus de 200 indices ; elle suppose une
  \textbf{nomenclature de bandes} (\texttt{N}, \texttt{R}, \texttt{G},
  \texttt{S1}\ldots).
\item
  \texttt{spyndex.computeIndex} applique un indice à partir des bandes
  nommées ; renommer les bandes (\texttt{assign\_coords}) facilite
  l'usage.
\item
  Le \textbf{NDVI} \(= (N - R)/(N + R)\) est élevé pour la végétation
  dense.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Calculez le \textbf{NDWI} (eau) et le \textbf{NDBI} (bâti) avec
  \texttt{spyndex} sur \texttt{img\_s2}, puis affichez-les côte à côte
  avec le NDVI.
\item
  Comparez les \textbf{signatures spectrales} de deux surfaces
  supplémentaires de la base USGS (p.~ex. neige, végétation sèche) sur
  les bandes Sentinel-2.
\item
  Parcourez \texttt{spyndex.indices}, choisissez un indice adapté à
  l'eau ou aux sols, identifiez les bandes qu'il requiert, et
  calculez-le sur \texttt{img\_s2}.
\item
  Renommez les bandes de \texttt{img\_s2} avec la nomenclature
  \texttt{spyndex} et vérifiez le résultat avec
  \texttt{img\_s2.coords{[}\textquotesingle{}band\textquotesingle{}{]}}.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{Que caractérise la signature spectrale d'une surface ?}
  ...
  \item
    Sa température
  \item
    Le type de matériau observé et son état
  \item
  ...
  ».
\item
  \textbf{Combien de bandes spectrales un capteur hyperspectral peut-il
  contenir ?}
  ...
  ».
\item
  \textbf{Quelle bibliothèque Python donne accès à plus de 200 indices
  spectraux ?}
  ...
  \item
    NumPy
  \item
    spyndex (Awesome Spectral Indices)
  \item
    rasterio
  \item
    xarray
...
\item
  \textbf{Dans la nomenclature de \texttt{spyndex}, quelle lettre
  désigne la bande du proche-infrarouge ?}
...
\item
  \textbf{Le NDVI se calcule à partir des bandes rouge et
  proche-infrarouge.}
...
\item
  \textbf{À quoi sert un indice spectral ?}

  \begin{itemize}
  \tightlist
  \item
    Mettre en valeur certaines caractéristiques du spectre (pentes,
    gradients)
  \item
    Améliorer la séparabilité de certaines classes d'intérêt
  \item
    Augmenter la résolution spatiale de l'image
  \item
    Corriger la géométrie de l'image
  \end{itemize}

  Relisez au besoin la section « Indices spectraux ».
\item
  \textbf{Quelle fonction de \texttt{spyndex} calcule un indice à partir
  des bandes ?}
  ...
  Relisez au besoin la section « Indices spectraux ».
\item
  \textbf{La signature spectrale peut être vue comme :}

  \begin{itemize}
  \tightlist
  \item
    Une généralisation de la couleur au-delà des bandes visibles
  \item
    Un histogramme des pixels de l'image
  \item
  ...
  ».
\item
  \textbf{Quel organisme maintient une base de données de signatures
  spectrales acquises en laboratoire ?}
...
\item
  ...
  \end{itemize}
\item
  Dans la nomenclature de \texttt{spyndex}, quelle lettre désigne la
  bande du proche-infrarouge ?
...
\item
  ...
  \end{itemize}
\item
  Quelle fonction de \texttt{spyndex} calcule un indice à partir des
  bandes ?
...
\item
  ...
  \end{itemize}
\item
  Quel organisme maintient une base de données de signatures spectrales
  acquises en laboratoire ?
...
pour le calcul fréquentiel (Cooley et Tukey 1965). La \emph{1D-DFT} peut
s'écrire simplement comme une projection sur une série d'exponentielles
complexes:

\begin{equation}\protect\phantomsection\label{eq-dft}{X[k] = \sum_{n=0 \ldots N-1} x[n] \times \exp(-j \times 2\pi \times k \times n/N))}\end{equation}

La transformée inverse prend une forme similaire:

\begin{equation}\protect\phantomsection\label{eq-idft}{x[k] = \frac{1}{N}\sum_{n=0 \ldots N-1} X[n] \times \exp(j \times 2\pi \times k \times n/N))}\end{equation}
...
près et forment un miroir des composantes, la convention est alors de
...
\(F_s\) du signal a aussi beaucoup d'importance et doit être au moins à
deux fois la plus haute fréquence observée (ici \(F_s > 0.5\)) sinon un
phénomène de repliement appelé aliasing sera observé.
...
\begin{equation}\protect\phantomsection\label{eq-fourier-filter}{
X_f[k] = F[k] \times X[k]
}\end{equation}
...
\begin{equation}\protect\phantomsection\label{eq-convolve}{
x_f = IDFT^{-1}[F]*x
}\end{equation}
...
\begin{equation}\protect\phantomsection\label{eq-boxfilter}{
F= \frac{1}{25}\left[
...
\CommentTok{\# notez la répétition du kernel 3x pour traiter une image rgb :}
\NormalTok{kernel }\OperatorTok{=}\NormalTok{ np.tile(kernel,(}\DecValTok{3}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{))}
...
\begin{equation}\protect\phantomsection\label{eq-scharr-filter}{
F= \left[
\begin{array}{ccc}
-3-3j & 0-10j & +3-3j \\
-10+0j & 0+0j & +10+0j \\
...
Cette section utilise la librairie PyTorch. Le calcul s'exécute très
bien sur CPU (c'est le cas lors de la génération de ce livre) ; un GPU,
par exemple sur Colab, ne fait que l'accélérer. Pour une installation
locale en version CPU : \texttt{pip\ install\ -qU\ torch==2.4.0+cpu}
...
\CommentTok{\# Filtre de Scharr (coefficients 3/10/3, cf. l\textquotesingle{}exemple de convolution ci{-}dessus)}
\NormalTok{scharr\_x }\OperatorTok{=}\NormalTok{ np.array([[}\OperatorTok{{-}}\DecValTok{3}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{3}\NormalTok{], [}\OperatorTok{{-}}\DecValTok{10}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{], [}\OperatorTok{{-}}\DecValTok{3}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{3}\NormalTok{]])}
\NormalTok{scharr\_y }\OperatorTok{=}\NormalTok{ np.array([[}\OperatorTok{{-}}\DecValTok{3}\NormalTok{, }\OperatorTok{{-}}\DecValTok{10}\NormalTok{, }\OperatorTok{{-}}\DecValTok{3}\NormalTok{], [}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{], [}\DecValTok{3}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{3}\NormalTok{]])}
\CommentTok{\# Le filtre (kernel) est formé de deux filtres}
\NormalTok{kernel }\OperatorTok{=}\NormalTok{ np.stack([scharr\_x, scharr\_y])}
...
\CommentTok{\# imshow (raster) plutôt que .plot() (pcolormesh vectoriel = PDF de 40 Mo)}
\NormalTok{xr.ufuncs.log10(img\_SAR.sel(band}\OperatorTok{=}\DecValTok{1}\NormalTok{).drop(}\StringTok{"band"}\NormalTok{)).plot.imshow()}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rolling\_win }\OperatorTok{=}\NormalTok{ img\_SAR.sel(band}\OperatorTok{=}\DecValTok{1}\NormalTok{).rolling(x}\OperatorTok{=}\DecValTok{5}\NormalTok{, y}\OperatorTok{=}\DecValTok{5}\NormalTok{,  min\_periods}\OperatorTok{=} \DecValTok{3}\NormalTok{, center}\OperatorTok{=} \VariableTok{True}\NormalTok{).construct(x}\OperatorTok{=}\StringTok{"x\_win"}\NormalTok{, y}\OperatorTok{=}\StringTok{"y\_win"}\NormalTok{, keep\_attrs}\OperatorTok{=} \VariableTok{True}\NormalTok{)}
\NormalTok{filtre\_moyen}\OperatorTok{=}\NormalTok{ rolling\_win.mean(dim}\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}x\_win\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}y\_win\textquotesingle{}}\NormalTok{], skipna}\OperatorTok{=} \VariableTok{True}\NormalTok{)}
\NormalTok{fig, axs }\OperatorTok{=}\NormalTok{ plt.subplots(}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{6}\NormalTok{, }\DecValTok{4}\NormalTok{))}
...
\begin{equation}\protect\phantomsection\label{eq-lee-filter}{ 
\begin{aligned}
...
En pratique, on exprime souvent la pondération \(K\) à l'aide du
\textbf{coefficient de variation} local \(CV = \sigma_I / I_M\) :
\(K = (CV - CV_{bruit})\,/\,CV\), où \(CV_{bruit}\) est le coefficient
de variation du \emph{speckle} (ici supposé \(\approx 0{,}25\)). C'est
cette forme équivalente qui est utilisée dans le code ci-dessous
(\texttt{ponderation}).
...
\section{Points clés}\label{points-cluxe9s-4}
...
\tightlist
\item
  La \textbf{transformée de Fourier} offre un point de vue fréquentiel
  réversible ; modifier le spectre filtre \textbf{globalement} l'image.
\item
  Trois familles de filtres fréquentiels : \textbf{passe-bas} (lissage),
  \textbf{passe-haut} (détails/contours) et \textbf{passe-bande}.
\item
  L'\textbf{aliasing} provient d'un sous-échantillonnage : appliquer un
  filtre passe-bas \textbf{avant} de décimer.
\item
  Le \textbf{filtrage local} par fenêtre glissante : filtre
  \textbf{moyen} (lissage), filtre de \textbf{Scharr} (contours) ; la
  \textbf{convolution} est efficace car calculable dans le domaine
  fréquentiel.
\item
  Le filtre de \textbf{Lee} est \textbf{adaptatif} : il préserve les
  détails (\(K \to 1\)) là où la variance locale est forte --- utile
  contre le \emph{speckle} des images SAR.
\item
  La \textbf{segmentation} (SLIC) réduit l'image en
  \textbf{super-pixels} homogènes ; le \textbf{graphe d'adjacence (RAG)}
  fusionne les segments ; l'\textbf{approche objet}
  (\texttt{regionprops}) décrit chaque segment par des propriétés.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Décimez \texttt{img\_rgb} par un facteur 8, \textbf{avec} et
  \textbf{sans} filtre passe-bas gaussien préalable, et comparez
  l'aliasing obtenu.
\item
  Appliquez le filtre de Scharr sur les \textbf{trois bandes} RVB
  (plutôt qu'une seule) et combinez les amplitudes de gradient (p.~ex.
  la norme euclidienne).
\item
  Faites varier la taille de la fenêtre (3×3, 7×7, 11×11) du filtre de
  Lee sur \texttt{img\_SAR} et observez l'effet sur la préservation des
  détails.
\item
  Reprenez SLIC avec plusieurs valeurs de \texttt{compactness} (1, 10,
  50) et décrivez l'effet sur la forme des super-pixels.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{La transformée de Fourier d'une image est complètement
  réversible.}
  ...
  Relisez au besoin la section « La transformée de Fourier ».
\item
  \textbf{À quoi sert un filtre passe-bas ?}

  \begin{itemize}
  \tightlist
  \item
    À ne préserver que les hautes fréquences (détails, contours)
  \item
    À lisser l'image en ne préservant que les basses fréquences
  \item
    À géoréférencer l'image
  \item
    À augmenter la résolution spatiale
  \end{itemize}

  Relisez au besoin la section « Filtrage fréquentiel ».
\item
  \textbf{Quelle est la cause de l'aliasing (repliement de spectre) ?}

  \begin{itemize}
  \tightlist
  \item
    Une fréquence d'échantillonnage trop élevée
  \item
    Une fréquence d'échantillonnage trop faible par rapport au contenu
    fréquentiel
  \item
    Un mauvais géoréférencement
  \item
    Un excès de bandes spectrales
  \end{itemize}

  Relisez au besoin la section « L'aliasing ».
\item
  \textbf{Une convolution dans l'espace image correspond à quelle
  ...
  Relisez au besoin la section « Filtrage fréquentiel ».
\item
  \textbf{Avant de décimer fortement une image, pourquoi appliquer un
  filtre passe-bas (p.~ex. gaussien) ?}
  ...
  \item
    Pour réduire les hautes fréquences et éviter l'aliasing
  \item
    Pour géoréférencer l'image
  \item
    Pour convertir l'image en dB
  \end{itemize}

  Relisez au besoin la section « L'aliasing ».
\item
  \textbf{À quoi sert le filtre de Scharr utilisé dans le chapitre ?}
  ...
  \item
    À lisser le bruit
  \item
    À égaliser l'histogramme
  \item
    À détecter les contours (gradients horizontaux et verticaux)
  \item
    À segmenter l'image en objets
  \end{itemize}

  Relisez au besoin la section « Filtrage par convolution ».
\item
  \textbf{Pour quel type d'images le filtre de Lee est-il
  particulièrement utile ?}

  \begin{itemize}
  \tightlist
  \item
    Les images optiques RVB
  \item
    Les images SAR dégradées par le speckle (bruit multiplicatif)
  \item
  ...
  Relisez au besoin la section « Filtrage adaptatif ».
\item
  \textbf{Dans le filtre de Lee, lorsque la variance locale est élevée,
  le coefficient K s'approche de 1, ce qui :}

  \begin{itemize}
  \tightlist
  \item
    préserve les détails de l'image d'origine
  \item
    applique la moyenne locale (lissage maximal)
  \item
    met le pixel à zéro
  \item
    inverse le contraste
  \end{itemize}

  Relisez au besoin la section « Filtrage adaptatif ».
\item
  \textbf{Sur quels critères la méthode SLIC regroupe-t-elle les pixels
  en super-pixels ?}

  \begin{itemize}
  \tightlist
  \item
    La similarité de couleur entre pixels
  \item
    La distance spatiale au centre du super-pixel
  \item
    La date d'acquisition de l'image
  \item
    Le système de projection cartographique
  \end{itemize}

  Relisez au besoin la section « Super-pixel ».
\item
  \textbf{Dans SLIC, quel est l'effet d'une valeur de compacité (m)
  élevée ?}

  \begin{itemize}
  \tightlist
  \item
    Les super-pixels sont plus réguliers/compacts et suivent moins le
    contenu de l'image
  \item
    Les super-pixels épousent parfaitement les contours des objets
  \item
    Le nombre de super-pixels augmente automatiquement
  \item
    La segmentation devient aléatoire
  \end{itemize}

  Relisez au besoin la section « Super-pixel ».
\item
  \textbf{À quoi sert un graphe d'adjacence de régions (RAG) après une
  segmentation ?}

  \begin{itemize}
  \tightlist
  \item
    À augmenter le nombre de segments
  \item
    À fusionner les segments similaires selon leur distance
    radiométrique
  \item
    À géoréférencer les segments
  \item
    À calculer la transformée de Fourier
  \end{itemize}

  Relisez au besoin la section « Fusion des segments par graphe de
  proximité ».
\item
  \textbf{Que permet l'approche objet avec \texttt{regionprops\_table}
  de \texttt{skimage} ?}

  \begin{itemize}
  \tightlist
  \item
    Extraire des propriétés par segment (aire, centroïde, intensité
    moyenne\ldots) dans un tableau
  \item
    Appliquer un filtre de Fourier
  \item
    Réduire le speckle
  \item
    Décimer l'image
  ...
  Relisez au besoin la section « Approche objet ».
\end{itemize}
...
\tightlist
\item
  La transformée de Fourier d'une image est complètement réversible.
...
\item
  ...
  \end{itemize}
\item
  Quelle est la cause de l'aliasing (repliement de spectre) ?
  ...
  \end{itemize}
\item
  Une convolution dans l'espace image correspond à quelle opération dans
  l'espace de Fourier ?
  ...
  \end{itemize}
\item
  Avant de décimer fortement une image, pourquoi appliquer un filtre
  passe-bas (p.~ex. gaussien) ?
  ...
  \end{itemize}
\item
  À quoi sert le filtre de Scharr utilisé dans le chapitre ?
  ...
  \end{itemize}
\item
  Pour quel type d'images le filtre de Lee est-il particulièrement utile
  ?
  ...
  \end{itemize}
\item
  Dans le filtre de Lee, lorsque la variance locale est élevée, le
  coefficient K s'approche de 1, ce qui :
  ...
  \end{itemize}
\item
  Sur quels critères la méthode SLIC regroupe-t-elle les pixels en
  super-pixels ?
  ...
  \end{itemize}
\item
  Dans SLIC, quel est l'effet d'une valeur de compacité (m) élevée ?
  ...
  \end{itemize}
\item
  À quoi sert un graphe d'adjacence de régions (RAG) après une
  segmentation ?
  ...
  \end{itemize}
\item
  Que permet l'approche objet avec \texttt{regionprops\_table} de
  \texttt{skimage} ?
...
\href{https://scikit-learn.org/}{\texttt{scikit-learn}}. Ce chapitre est
...
\item
  mettre en place des techniques de classifications comme K-NN et les
  arbres de décision;
\end{itemize}
...
Cet exemple tiré de
\href{https://scikit-learn.org/stable/auto_examples/model_selection/plot_underfitting_overfitting.html\#sphx-glr-auto-examples-model-selection-plot-underfitting-overfitting-py}{\texttt{scikit-learn}}
...
incapables de déterminer la situation de sur-apprentissage, l'erreur sur
les points d'entraînement seuls étant excellente pour un degré 15.
...
\item
  Partage des données en trois groupes: entraînement, validation et
  test.
\item
...
\item
  ...
  à vérifier que les hyper-paramètres du modèle sont adéquats.
\item
...
On peut facilement calculer la fréquence d'occurrences des 12 classes
...
Une fois les points sélectionnés, on ajoute les valeurs des bandes
provenant d'une image satellite. Pour cela, on utilise la méthode
...
l'information de géoréférence. Afin de pouvoir classifier ces points, on
ajoute les valeurs radiométriques provenant de l'image Sentinel-2 à 4
bandes \texttt{RGBNIR\_of\_S2A.tif}. Ces valeurs seront stockées dans la
colonne \texttt{value} sous la forme d'un vecteur en format
\texttt{string} :
...
simulées ainsi que les distances JM correspondantes :
...
\NormalTok{class\_labels }\OperatorTok{=}\NormalTok{ np.unique(y).tolist() }\CommentTok{\# étiquettes présentes (triées ; certaines classes peuvent être absentes)}
\NormalTok{n\_classes }\OperatorTok{=} \BuiltInTok{len}\NormalTok{(class\_labels)}
\CommentTok{\# remappe les étiquettes vers 0..n{-}1, aligné ligne à ligne avec X (toujours défini, sans dépendre de l\textquotesingle{}ordre des données)}
\NormalTok{y\_new }\OperatorTok{=}\NormalTok{ np.searchsorted(class\_labels, y)}
...
d'entrainement de 10 classes. On obtient un tableau symétrique de 10x10
...
ce qui nous permet de constater que la classe eau a le maximum de
séparabilité avec les autres classes.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ parallelepiped\_train(X\_train, y\_train):}
...
\NormalTok{clf}\OperatorTok{=}\NormalTok{ parallelepiped\_train(X, y\_new)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{def}\NormalTok{ parallelepiped\_predict(clf, X\_test):}
...
\NormalTok{        inside }\OperatorTok{=}\NormalTok{ inside }\KeywordTok{and}\NormalTok{ (data[b] }\OperatorTok{\textgreater{}=}\NormalTok{ limit[}\DecValTok{0}\NormalTok{]) }\OperatorTok{\&}\NormalTok{ (data[b] }\OperatorTok{\textless{}=}\NormalTok{ limit[}\DecValTok{1}\NormalTok{])}
        \ControlFlowTok{if} \KeywordTok{not}\NormalTok{ inside:}
...
\NormalTok{y\_image}\OperatorTok{=}\NormalTok{ parallelepiped\_predict(clf, data\_image)}
...
\NormalTok{ax.set\_title(}\StringTok{"Méthode des parallélépipèdes"}\NormalTok{, fontsize}\OperatorTok{=}\StringTok{"small"}\NormalTok{)}
\NormalTok{plt.show()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{y\_pred}\OperatorTok{=}\NormalTok{ parallelepiped\_predict(clf, X)}
\NormalTok{nom\_classes2}\OperatorTok{=}\NormalTok{ [nom\_classes[c] }\ControlFlowTok{for}\NormalTok{ c }\KeywordTok{in}\NormalTok{ np.unique(y).tolist()]}
...
choisira un nombre impair pour la valeur de \(K\). Malgré sa simplicité,
cette technique peut devenir assez demandante en temps de calcul pour un
nombre important de points et un nombre élevé de dimensions.
...
poids dans le calcul des distances entre points. Cette opération porte
le nom de \texttt{StandardScaler} dans \texttt{scikit-learn}. On peut
alors former un pipeline de traitement combinant les deux opérations :
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{05-ClassificationsSupervisees_files/figure-pdf/cell-28-output-2.pdf}}

}

\caption{Frontières de décision pour le classificateur K-NN}
...
Trees}). Les arbres de décision on tendance à surapprendre surtout si le
nombre de dimensions est élevé. Il est donc conseillé d'avoir un bon
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{05-ClassificationsSupervisees_files/figure-pdf/cell-35-output-2.pdf}}

}

\caption{Frontières de décision pour des arbres de décision de
...
Il est possible de visualiser l'arbre avec l'outil
\href{https://github.com/mljar/supertree?tab=readme-ov-file}{SuperTree}
mais cela contient beaucoup d'information

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ supertree }\ImportTok{import}\NormalTok{ SuperTree }\CommentTok{\# \textless{}{-} import supertree :)}
\NormalTok{super\_tree }\OperatorTok{=}\NormalTok{ SuperTree(clf, X\_train, y\_train, [}\StringTok{\textquotesingle{}Bleu\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}Vert\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}Rouge\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}NIR\textquotesingle{}}\NormalTok{], nom\_classes2)}

\NormalTok{super\_tree.show\_tree()}
\end{Highlighting}
\end{Shaded}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{05-ClassificationsSupervisees_files/figure-pdf/cell-43-output-1.pdf}}

}

\caption{Frontières de décision pour un classificateur Bayésien naif}

\end{figure}%

On observe que les frontières de décision sont beaucoup plus régulières
que pour K-NN.
...
\section{Réseaux de neurones}\label{ruxe9seaux-de-neurones}

Les réseaux de neurones artificiels (RNA) ont connu un essor très
important depuis les années 2010 avec des approches dites profondes. Ces
aspects seront surtout abordés dans le tome 2 consacré à l'intelligence
artificielle. On aborde ici seulement le \textbf{perceptron} et le
\textbf{perceptron multi-couches} (MLP).

Le perceptron est l'unité de base d'un RNA : il consiste en \(N\)
connexions, une unité de calcul (le neurone) avec une fonction
d'activation et une sortie. Le perceptron ne permet de construire que
des frontières de décision \textbf{linéaires}.

Le perceptron multi-couches est un réseau dense (\emph{fully connected})
avec des \textbf{couches cachées} entre la couche d'entrée et la couche
de sortie, ce qui permet de construire des frontières de décision
beaucoup plus complexes via une hiérarchie de frontières de décision.
Ces réseaux sont entraînés par descente de gradient avec une correction
des poids par \textbf{rétro-propagation} de l'erreur, mesurée par une
fonction de coût que l'on cherche à réduire.

Comme pour le K-NN, il est important de \textbf{normaliser} les données
(\texttt{StandardScaler}) avant d'entraîner un MLP. On forme donc un
pipeline combinant la normalisation et le classificateur :
...
\NormalTok{clf }\OperatorTok{=}\NormalTok{ Pipeline(}
\NormalTok{    steps}\OperatorTok{=}\NormalTok{[(}\StringTok{"scaler"}\NormalTok{, StandardScaler()),}
\NormalTok{           (}\StringTok{"mlp"}\NormalTok{, MLPClassifier(hidden\_layer\_sizes}\OperatorTok{=}\NormalTok{(}\DecValTok{50}\NormalTok{, }\DecValTok{50}\NormalTok{), max\_iter}\OperatorTok{=}\DecValTok{1000}\NormalTok{, random\_state}\OperatorTok{=}\DecValTok{0}\NormalTok{))]}
\NormalTok{)}
\NormalTok{clf.fit(X\_train, y\_train)}
\NormalTok{y\_pred }\OperatorTok{=}\NormalTok{ clf.predict(X\_test)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Nombre de points misclassifiés sur }\SpecialCharTok{\%d}\StringTok{ points : }\SpecialCharTok{\%d}\StringTok{"}
...
\begin{verbatim}
Nombre de points misclassifiés sur 200 points : 112
\end{verbatim}

En faisant varier l'architecture (nombre et taille des couches cachées),
on observe le passage d'une frontière linéaire (perceptron sans couche
cachée) à des frontières de plus en plus complexes :

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{05-ClassificationsSupervisees_files/figure-pdf/cell-50-output-1.pdf}}

}

\caption{Frontières de décision d'un MLP selon l'architecture (bandes
Rouge et NIR)}

\end{figure}%

Le perceptron sans couche cachée produit une frontière linéaire,
comparable à un classificateur linéaire, tandis que l'ajout de couches
cachées permet des frontières non linéaires plus souples --- au risque
de sur-apprendre.

Le rapport de performance et la matrice de confusion du modèle à deux
couches (sur les 4 bandes) :

\begin{verbatim}
                       precision    recall  f1-score   support

           Commercial       0.43      0.40      0.41        15
                Foret       0.44      0.73      0.55        11
    Faible_végétation       0.23      0.11      0.15        27
               Sol_nu       0.52      0.50      0.51        22
                Roche       0.50      0.30      0.38        23
                Route       0.24      0.28      0.26        18
               Urbain       0.33      0.30      0.32        20
                  Eau       0.92      0.96      0.94        24
    Végétation éparse       0.30      0.60      0.40        15
Roche avec végétation       0.38      0.40      0.39        25
...
\pandocbounded{\includegraphics[keepaspectratio]{05-ClassificationsSupervisees_files/figure-pdf/cell-52-output-1.pdf}}

Enfin, la prédiction peut s'appliquer sur toute l'image :

\pandocbounded{\includegraphics[keepaspectratio]{05-ClassificationsSupervisees_files/figure-pdf/cell-53-output-1.pdf}}

\section{Points clés}\label{points-cluxe9s-5}
...
\tightlist
\item
  La \textbf{classification supervisée} attribue une classe à chaque
  pixel à partir d'un \textbf{jeu d'entraînement étiqueté} ; le modèle
  apprend des \textbf{frontières de décision}.
\item
  Séparer les données en \textbf{entraînement / validation / test} est
  essentiel pour détecter le \textbf{sur-apprentissage} --- l'erreur
  d'entraînement seule est trompeuse.
\item
  Les ensembles réels sont souvent \textbf{déséquilibrés} ; la
  \textbf{séparabilité} (distance JM) et les statistiques par classe
  guident l'analyse préliminaire.
\item
  La performance se lit dans la \textbf{matrice de confusion} :
  \textbf{rappel} (omission, lignes), \textbf{précision} (fausses
  alarmes, colonnes), \textbf{f1-score}.
\item
  Méthodes \textbf{non paramétriques} (parallélépipèdes, K-NN, arbres)
  vs \textbf{paramétriques} (Bayésien naïf, ADQ) ; le K-NN exige une
  \textbf{normalisation} (\texttt{StandardScaler}).
\item
  La \textbf{malédiction de la dimensionnalité} : le nombre de points
  requis croît exponentiellement avec le nombre de dimensions \(D\).
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Comparez les rapports de performance (\texttt{classification\_report})
  du K-NN, de l'arbre de décision et de l'ADQ sur le même ensemble de
  test : quelle méthode l'emporte, et pour quelles classes ?
\item
  Faites varier \texttt{test\_size} (0.1, 0.3, 0.5) dans
  \texttt{train\_test\_split} et observez l'effet sur l'erreur de test
  du K-NN.
\item
  Entraînez un arbre de décision avec plusieurs \texttt{max\_depth} (3,
  5, 10, \texttt{None}) et repérez à partir de quelle profondeur le
  sur-apprentissage apparaît.
\item
  Utilisez les \textbf{4 bandes} (au lieu de Rouge--NIR) pour l'ADQ et
  comparez la carte d'occupation des sols obtenue.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{En quoi consiste une classification supervisée ?}

  \begin{itemize}
  \tightlist
  \item
    Regrouper les pixels sans aucune étiquette de référence
  \item
    Attribuer une classe à chaque point à partir d'un jeu d'entraînement
    étiqueté
  \item
    Réduire le nombre de bandes d'une image
  \item
    Corriger la géométrie d'une image
  \end{itemize}

  Relisez au besoin la section « Principes généraux ».
\item
  \textbf{Qu'est-ce que le sur-apprentissage (overfitting) ?}

  \begin{itemize}
  \tightlist
  \item
    Le modèle apprend le bruit des données d'entraînement et généralise
    mal
  \item
    Le modèle est trop simple pour s'ajuster aux données
  \item
    Le modèle n'a pas assez de dimensions
  \item
    Le modèle ignore l'ensemble d'entraînement
  \end{itemize}

  Relisez au besoin la section « Comportement d'un modèle ».
\item
  \textbf{Pourquoi a-t-on besoin d'un ensemble de validation/test
  ...
  \item
    Pour augmenter le nombre de classes
  \item
    Parce que l'erreur sur l'entraînement seul ne révèle pas le
    sur-apprentissage
  \item
    Pour normaliser les données
  \end{itemize}

  Relisez au besoin les sections « Comportement d'un modèle » et «
  Pipeline ».
\item
  \textbf{Un ensemble d'entraînement où les classes ont des fréquences
  très différentes est dit :}
  ...
  Relisez au besoin la section « Construction d'un ensemble
  d'entraînement ».
\item
  \textbf{Que mesure la distance de Jeffries-Matusita (JM) entre deux
  classes ?}
  ...
  \item
    Le nombre d'échantillons par classe
  \item
    La séparabilité des classes sous l'hypothèse de modèles gaussiens
  \item
    La résolution spatiale de l'image
  \item
    Le temps de calcul du modèle
  \end{itemize}

  Relisez au besoin la section « Analyse préliminaire des données ».
\item
  \textbf{Dans la matrice de confusion, le rappel (recall) d'une classe
  correspond à :}

  \begin{itemize}
  \tightlist
  \item
    une normalisation par rapport aux lignes (sensible aux erreurs
    d'omission)
  \item
    une normalisation par rapport aux colonnes (sensible aux fausses
    alarmes)
  \item
    la somme totale de la matrice
  \item
    la diagonale divisée par le nombre de classes
  \end{itemize}

  Relisez au besoin la section « Mesures de performance ».
\item
  \textbf{Le f1-score est :}
  ...
  \item
    le nombre de points bien classés
  \end{itemize}

  Relisez au besoin la section « Mesures de performance ».
\item
  \textbf{Quel est le principe de la méthode des parallélépipèdes ?}

  \begin{itemize}
  \tightlist
  \item
    Un vote parmi les K voisins les plus proches
  \item
    Délimiter le domaine de chaque classe par une boîte min/max par
    bande
  \item
    Ajuster une gaussienne multivariée par classe
  \item
    Construire une cascade de règles de seuil
  \end{itemize}

  Relisez au besoin la section « Méthode des parallélépipèdes ».
\item
  \textbf{Qu'est-ce que la malédiction de la dimensionnalité ?}

  \begin{itemize}
  \tightlist
  \item
    Le nombre de points nécessaires croît exponentiellement avec le
    nombre de dimensions
  \item
    Les images deviennent trop grandes à afficher
  \item
    Le nombre de classes augmente avec les bandes
  \item
    Les couleurs se mélangent au-delà de trois bandes
  \end{itemize}

  Relisez au besoin la section « La malédiction de la haute dimension ».
\item
  \textbf{Pourquoi centre-t-on et réduit-on (StandardScaler) les données
  avant un K-NN ?}

  \begin{itemize}
  \tightlist
  \item
    Pour que chaque dimension ait le même poids dans le calcul des
    distances
  \item
  ...
  \item
    Pour supprimer les points aberrants
  \end{itemize}

  Relisez au besoin la section « Plus proches voisins ».
\item
  \textbf{Dans un K-NN, augmenter la valeur de K :}

  \begin{itemize}
  \tightlist
  \item
    n'a aucun effet sur les frontières de décision
  \item
    rend les frontières de décision plus lisses (et le calcul plus long)
  \item
    garantit toujours une meilleure précision
  \item
  ...
  Relisez au besoin la section « Plus proches voisins ».
\item
  \textbf{Quelle hypothèse simplificatrice fait la méthode Bayésienne
  naïve ?}

  \begin{itemize}
  \tightlist
  \item
    Les classes ont toutes la même fréquence
  \item
    Les frontières de décision sont linéaires
  \item
    Les dimensions (bandes) sont indépendantes
  \item
    Toutes les classes ont la même covariance
  \end{itemize}

  Relisez au besoin la section « Méthode Bayésienne naïve ».
\item
  \textbf{La différence entre une méthode paramétrique et non
  paramétrique est :}

  \begin{itemize}
  \tightlist
  \item
    le nombre de paramètres d'une méthode paramétrique est fixe et ne
    dépend pas de la taille des données
  \item
    une méthode paramétrique n'a pas besoin d'entraînement
  \item
    une méthode non paramétrique ne fait que du seuillage
  \item
    il n'y a aucune différence
  \end{itemize}

  Relisez au besoin les sections « Méthodes non paramétriques » et «
...
\tightlist
\item
  En quoi consiste une classification supervisée ?
...
\item
  ...
  \end{itemize}
\item
  Pourquoi a-t-on besoin d'un ensemble de validation/test distinct de
  l'entraînement ?
  ...
  \end{itemize}
\item
  Un ensemble d'entraînement où les classes ont des fréquences très
  différentes est dit :
  ...
  \end{itemize}
\item
  Que mesure la distance de Jeffries-Matusita (JM) entre deux classes ?
  ...
  \end{itemize}
\item
  Dans la matrice de confusion, le rappel (recall) d'une classe
  correspond à :
...
\item
  ...
  \end{itemize}
\item
  Quel est le principe de la méthode des parallélépipèdes ?
...
\item
  ...
  \end{itemize}
\item
  Pourquoi centre-t-on et réduit-on (StandardScaler) les données avant
  un K-NN ?
  ...
  \end{itemize}
\item
  Dans un K-NN, augmenter la valeur de K :
  ...
  \end{itemize}
\item
  Quelle hypothèse simplificatrice fait la méthode Bayésienne naïve ?
  ...
  \end{itemize}
\item
  La différence entre une méthode paramétrique et non paramétrique est :
...
Sciences}} 8: 1430. \url{https://doi.org/10.3390/app8091430}.
...
Sciences}}. \url{https://doi.org/10.5066/F7RR1WDJ}.
