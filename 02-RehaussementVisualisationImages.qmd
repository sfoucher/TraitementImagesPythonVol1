...
::: bloc_attention-body
Les cartes `leafmap` sont des composants **JavaScript** : elles s'affichent dans un **notebook** (Google Colab, Jupyter) ou dans la **version HTML** de ce livre, mais **pas dans le PDF**. Les cellules ci-dessous ne sont donc pas exécutées à la compilation (`#| eval: false`) — ouvrez ce chapitre dans [![](images/colab.png)](https://colab.research.google.com/github/sfoucher/TraitementImagesPythonVol1/blob/main/notebooks/02-RehaussementVisualisationImages.ipynb) pour les manipuler.
:::
::::::
